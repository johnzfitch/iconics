...
fn render_grid(f: &mut Frame, app: &mut App, area: Rect, is_focused: bool) {
    ...
    if app.grid_cols != cols || app.grid_rows != rows {
        app.grid_cols = cols;
        app.grid_rows = rows;
        app.grid_needs_refresh = true;
    }
...
fn render_grid_cell(f: &mut Frame, app: &mut App, area: Rect, filtered_idx: usize, is_selected: bool) {
    ...
    let label = icon.semantic_name.chars().take(8).collect::<String>();
    ...
    if let Some(protocol) = app.grid_image_states.get_mut(&icon.id) {

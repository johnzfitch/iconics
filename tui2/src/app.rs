...
impl App {
    ...
    pub fn toggle_basket(&mut self) {
        if let Some(icon) = self.get_selected_icon() {
            ...
            if let Some(pos) = self.basket.iter().position(|x| x == &id) {
                ...
                self.set_status(format!("Basket: removed '{name}'"));
            } else {
                self.basket.push(id);
                self.log(LogLevel::Info, format!("Added '{}' to basket", name));
                self.set_status(format!("Basket: added '{name}'"));
            }
        }
    }
    ...
    pub fn export_basket_to_clipboard(&mut self) -> Result<()> {
        ...
        let methods = self.set_clipboard_text(&markdown)?;
        self.set_status(format!(
            "Copied {} icon(s) to clipboard (markdown) via {}",
            self.basket.len(),
            methods.join(", ")
        ));
        self.log(
            LogLevel::Info,
            format!(
                "Exported {} icons to clipboard as markdown ({})",
                self.basket.len(),
                methods.join(", ")
            ),
        );
    ...
    fn try_set_clipboard_with_command(
            ...
            .spawn()
        {
            ...
            Err(err) => return Err(err).with_context(|| format!("Failed to spawn {program}")),
        };
        ...
        if !output.status.success() {
            ...
            if !stderr.is_empty() {
                self.log(
                    LogLevel::System,
                    format!("{program} failed: {}", stderr.lines().next().unwrap_or("unknown error")),
                );
            } else {
                self.log(LogLevel::System, format!("{program} failed with exit {}", output.status));
            }
            return Ok(false);
        }
